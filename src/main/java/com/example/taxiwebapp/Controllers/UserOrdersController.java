...
public class UserOrdersController {
    ...
    @Autowired
    public UserOrdersController(UsersRepository usersRepository, TaxiOrderRepository taxiOrderRepository, StatusRepository statusRepository) {
        this.usersRepository = usersRepository;
        this.taxiOrderRepository = taxiOrderRepository;
        this.statusRepository = statusRepository;
    }

    @GetMapping("/showOrdersHistory")
    public String getTaxiOrders(Model model) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String username = authentication.getName();
        UsersEntity user = usersRepository.findByUsername(username);
        Long userId = user.getId();
        List<TaxiOrderEntity> taxiOrders = taxiOrderRepository.findTaxiOrderEntitiesBySenderId(userId);
