...
@Controller
public class OrderToTakeController {
    private UsersRepository usersRepository;
    private StatusRepository statusRepository;
    private TaxiOrderRepository taxiOrderRepository;

    public OrderToTakeController(UsersRepository usersRepository, StatusRepository statusRepository, TaxiOrderRepository taxiOrderRepository) {
    ...
    public String getOrderDetails(@RequestParam("orderId") Long orderId, @RequestParam(value = "error",required = false) String error, Model model) {
        ...
        return "orderToTakePage";
    }

    @PostMapping("/orderToTake")
    public String cancelOrder(@RequestParam("orderId") Long orderId, @RequestParam(value = "error", required = false) String error, RedirectAttributes redirectAttributes) {
        ...
        List<TaxiOrderEntity> taxiOrderEntities = taxiOrderRepository.findTaxiOrderEntitiesByRecipientIdAndStatusId(usersEntity.getId(),4L);

        if (taxiOrderEntities.size()>1) {
            redirectAttributes.addAttribute("error", "У вас уже есть активный заказ");
            redirectAttributes.addAttribute("orderId", orderId);
            return "redirect:/orderToTake";
        }
        else {
            order.setStatusId(4L);
            order.setRecipientId(usersEntity.getId());
